...
public class wc {
    ...
    public static boolean isCommentEnd(String s){
        //寻找多行注释结束符号
        for(int i = 0; i < s.length() - 1; i++){
            if(s.charAt(i) == '*'){
                if(s.charAt(i + 1) == '/'){
    ...
    public static void getWC(File file) throws IOException {
        ...
        if (file.isFile() && file.getName().endsWith(".c")) {
            ...
            {
                //System.out.print(s + "\n");
                if(isCommentLine(s) || isCommentStart == 1){
                    countComment++;
                }
                else if(isCommentEnd(s)){
                    countComment++;
                    isCommentStart = 0;
                }
                ...
                for(int i = 0; i < tStr.length; i++){
                    if((eFlag == 1 && !isInStopList(tStr[i]) && !tStr[i].isEmpty()) || (eFlag == 0 && !tStr[i].isEmpty())){
                        countWord++;
                    }
                }
                countLine++;
            ...
            if(cFlag == 1){
                System.out.println(iPath + "，字符数：" + countChar);
                countChar = 0;
            }
            if(wFlag == 1){
